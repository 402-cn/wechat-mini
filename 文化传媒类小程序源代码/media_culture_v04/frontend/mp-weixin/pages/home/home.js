...
async loadSwipers() {
    await this._mpLoadSwiper_swiper_media_culture_v04_home_02()
  },
  async _mpLoadSwiper_swiper_media_culture_v04_home_02() {
    const { req, assetUrl } = require('../../utils/api');
    try {
      var j = await req('/swiper/list.php?id=' + encodeURIComponent("media_culture_v04_home_02"));
      ...
      var cur = this.data.swiper_media_culture_v04_home_02 || {};
      this.setData({
        swiper_media_culture_v04_home_02: {
    ...
    } catch (e) { this.mpDevWarn('swiper', "media_culture_v04_home_02", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.articles_media_culture_v04_home_07, 'cover'); if (list.length) this.setData({ articles_media_culture_v04_home_07: list }); }
    { var wgt = this.data.swiper_media_culture_v04_home_02; if (wgt && wgt.items && wgt.items.length) { var next = Object.assign({}, wgt, { items: withSrc(wgt.items, 'image') }); this.setData({ swiper_media_culture_v04_home_02: next }); } }
  },
goArticle(e) {
...
async loadArticles() {
    ...
    try {
      await this._mpLoadArticles_articles_media_culture_v04_home_07()
    } finally {
      this._articleLoading = false;
    }
  },
  async _mpLoadArticles_articles_media_culture_v04_home_07() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/article/list.php?limit=5&component_id=' + encodeURIComponent("media_culture_v04_home_07");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.articles_media_culture_v04_home_07 || []).slice();
      }
      if (!list.length) return;
      this.setData({ articles_media_culture_v04_home_07: list });
    } catch (e) {
      this.mpDevWarn('article', "media_culture_v04_home_07", e);
      var cur = this.data.articles_media_culture_v04_home_07;
      if (cur && cur.length) {
        try {
          ...
          this.setData({ articles_media_culture_v04_home_07: patched });
        } catch (e2) {}
      }
    }
  },
