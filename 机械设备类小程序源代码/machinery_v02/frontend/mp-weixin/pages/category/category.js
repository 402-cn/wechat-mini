...
async loadProducts() {
    await this._mpLoadProducts_products_machinery_v02_cat_04()
  },
  async _mpLoadProducts_products_machinery_v02_cat_04() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=8&component_id=' + encodeURIComponent("machinery_v02_cat_04");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_machinery_v02_cat_04 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_machinery_v02_cat_04: list });
    } catch (e) { this.mpDevWarn('product', "machinery_v02_cat_04", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_machinery_v02_cat_04, 'image'); if (list.length) this.setData({ products_machinery_v02_cat_04: list }); }
  },
onShow() {
