...
noticeDuration(speed) {
  ...
  async loadNotices() {
    await this._mpLoadNotice_notice_medical_v02_coupon_02()
  },
  async _mpLoadNotice_notice_medical_v02_coupon_02() {
    const { req } = require('../../../utils/api');
    try {
      var j = await req('/notice/get.php?id=' + encodeURIComponent("medical_v02_coupon_02"));
      if (!j || j.code !== 0 || !j.data || !j.data.content) return;
      var d = j.data;
      var speed = d.scrollSpeed || 50;
      var dir = d.scrollDirection || 'left';
      this.setData({
        notice_medical_v02_coupon_02: {
          content: d.content || '',
    ...
    } catch (e) { this.mpDevWarn('notice', "medical_v02_coupon_02", e); }
  },
