...
Page({
  data: { appModalShow: false, appModalTitle: '提示', appModalContent: '',  siteRoot: __mpSiteRoot, assetRoot: __mpAssetRoot, products_agriculture_v01_cart_03: [{"id":1,"image":"/uploads/stock/agriculture_10.jpg","imageSrc":"","name":"香菇干货","price":29},{"id":2,"image":"/uploads/stock/agriculture_11.jpg","imageSrc":"","name":"木耳","price":68},{"id":3,"image":"/uploads/stock/agriculture_12.jpg","imageSrc":"","name":"红枣","price":107},{"id":4,"image":"/uploads/stock/agriculture_13.jpg","imageSrc":"","name":"桂圆","price":146}], productWidgets: [{"key":"products_agriculture_v01_cart_03","cid":"agriculture_v01_cart_03","limit":4}], notice_agriculture_v01_cart_02: {"bgColor":"#fff8e1","content":"满99元包邮，请在结算前确认商品信息","duration":"5.50s","fontSize":26,"prefixTitle":"","scrollDirection":"left","scrollSpeed":40,"showIcon":true,"textColor":"#666","trackClass":"to-left"}, noticeWidgets: [{"key":"notice_agriculture_v01_cart_02","cid":"agriculture_v01_cart_02"}], cartLoggedIn: false, cartList: [], cartTotal: 0, cartSubtitle: "共0件商品" },
  onLoad(q) {
...
async loadProducts() {
    await this._mpLoadProducts_products_agriculture_v01_cart_03()
  },
  async _mpLoadProducts_products_agriculture_v01_cart_03() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=4&component_id=' + encodeURIComponent("agriculture_v01_cart_03");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_agriculture_v01_cart_03 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_agriculture_v01_cart_03: list });
    } catch (e) { this.mpDevWarn('product', "agriculture_v01_cart_03", e); }
  },
noticeDuration(speed) {
  ...
  async loadNotices() {
    await this._mpLoadNotice_notice_agriculture_v01_cart_02()
  },
  async _mpLoadNotice_notice_agriculture_v01_cart_02() {
    const { req } = require('../../utils/api');
    try {
      var j = await req('/notice/get.php?id=' + encodeURIComponent("agriculture_v01_cart_02"));
      if (!j || j.code !== 0 || !j.data || !j.data.content) return;
      var d = j.data;
      var speed = d.scrollSpeed || 50;
      var dir = d.scrollDirection || 'left';
      this.setData({
        notice_agriculture_v01_cart_02: {
          content: d.content || '',
    ...
    } catch (e) { this.mpDevWarn('notice', "agriculture_v01_cart_02", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_agriculture_v01_cart_03, 'image'); if (list.length) this.setData({ products_agriculture_v01_cart_03: list }); }
  },
onShow() {
