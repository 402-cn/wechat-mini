...
async loadProducts() {
    await this._mpLoadProducts_products_wedding_v07_cart_03()
  },
  async _mpLoadProducts_products_wedding_v07_cart_03() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=4&component_id=' + encodeURIComponent("wedding_v07_cart_03");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_wedding_v07_cart_03 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_wedding_v07_cart_03: list });
    } catch (e) { this.mpDevWarn('product', "wedding_v07_cart_03", e); }
  },
noticeDuration(speed) {
  ...
  async loadNotices() {
    await this._mpLoadNotice_notice_wedding_v07_cart_02()
  },
  async _mpLoadNotice_notice_wedding_v07_cart_02() {
    const { req } = require('../../utils/api');
    try {
      var j = await req('/notice/get.php?id=' + encodeURIComponent("wedding_v07_cart_02"));
      if (!j || j.code !== 0 || !j.data || !j.data.content) return;
      var d = j.data;
      var speed = d.scrollSpeed || 50;
      var dir = d.scrollDirection || 'left';
      this.setData({
        notice_wedding_v07_cart_02: {
          content: d.content || '',
    ...
    } catch (e) { this.mpDevWarn('notice', "wedding_v07_cart_02", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_wedding_v07_cart_03, 'image'); if (list.length) this.setData({ products_wedding_v07_cart_03: list }); }
  },
onShow() {
