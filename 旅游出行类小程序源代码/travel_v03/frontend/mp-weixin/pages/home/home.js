...
async loadProducts() {
    await this._mpLoadProducts_products_travel_v03_home_08()
  },
  async _mpLoadProducts_products_travel_v03_home_08() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=6&component_id=' + encodeURIComponent("travel_v03_home_08");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_travel_v03_home_08 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_travel_v03_home_08: list });
    } catch (e) { this.mpDevWarn('product', "travel_v03_home_08", e); }
  },
...
async loadSwipers() {
    await this._mpLoadSwiper_swiper_travel_v03_home_02()
  },
  async _mpLoadSwiper_swiper_travel_v03_home_02() {
    const { req, assetUrl } = require('../../utils/api');
    try {
      var j = await req('/swiper/list.php?id=' + encodeURIComponent("travel_v03_home_02"));
      ...
      var cur = this.data.swiper_travel_v03_home_02 || {};
      this.setData({
        swiper_travel_v03_home_02: {
    ...
    } catch (e) { this.mpDevWarn('swiper', "travel_v03_home_02", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_travel_v03_home_08, 'image'); if (list.length) this.setData({ products_travel_v03_home_08: list }); }
    { var wgt = this.data.swiper_travel_v03_home_02; if (wgt && wgt.items && wgt.items.length) { var next = Object.assign({}, wgt, { items: withSrc(wgt.items, 'image') }); this.setData({ swiper_travel_v03_home_02: next }); } }
  },
onShow() {
