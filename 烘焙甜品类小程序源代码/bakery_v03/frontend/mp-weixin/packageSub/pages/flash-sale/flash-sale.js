...
async loadProducts() {
    await this._mpLoadProducts_products_flash_sale_locked_product()
  },
  async _mpLoadProducts_products_flash_sale_locked_product() {
    const { req, assetUrl } = require('../../../utils/api');
    var url = '/product/list.php?limit=6&component_id=' + encodeURIComponent("flash_sale_locked_product");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_flash_sale_locked_product || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_flash_sale_locked_product: list });
    } catch (e) { this.mpDevWarn('product', "flash_sale_locked_product", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_flash_sale_locked_product, 'image'); if (list.length) this.setData({ products_flash_sale_locked_product: list }); }
  },
onMpTabSwitch(e) {
