...
$pdo = db();
$stmt = $pdo->prepare('SELECT target_id, viewed_at FROM user_footprints WHERE user_id=? AND visitor_key=? AND target_type=? ORDER BY viewed_at DESC LIMIT 200');
...
foreach ($rows as $r) {
    ...
    if ($type === 'product' && $pdo->query("SHOW TABLES LIKE 'products'")->fetch()) {
        ...
        if ($p) { $p['viewed_at'] = $r['viewed_at']; $list[] = $p; }
        ...
        if ($a) { $a['viewed_at'] = $r['viewed_at']; $list[] = $a; }
    }
}
