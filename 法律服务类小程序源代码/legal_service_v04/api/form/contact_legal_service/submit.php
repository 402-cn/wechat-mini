...
try {
    $pdo = db();
    ensure_form_table_columns($pdo, 'contact_legal_service', array(
        ['key'=>'name','type'=>'text','label'=>'姓名','options'=>array()],
        ['key'=>'phone','type'=>'phone','label'=>'手机号','options'=>array()],
        ['key'=>'message','type'=>'textarea','label'=>'留言内容','options'=>array()]
    ));
    $stmt = $pdo->prepare('INSERT INTO `contact_legal_service` (`name`, `phone`, `message`) VALUES (?, ?, ?)');
    $stmt->execute($values);
