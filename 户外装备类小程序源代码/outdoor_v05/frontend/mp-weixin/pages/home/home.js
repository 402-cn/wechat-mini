...
async loadProducts() {
    await this._mpLoadProducts_products_outdoor_v05_home_04();
    await this._mpLoadProducts_products_outdoor_v05_home_06()
  },
  async _mpLoadProducts_products_outdoor_v05_home_04() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=8&component_id=' + encodeURIComponent("outdoor_v05_home_04");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_outdoor_v05_home_04 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_outdoor_v05_home_04: list });
    } catch (e) { this.mpDevWarn('product', "outdoor_v05_home_04", e); }
  },

  async _mpLoadProducts_products_outdoor_v05_home_06() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=4&component_id=' + encodeURIComponent("outdoor_v05_home_06");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_outdoor_v05_home_06 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_outdoor_v05_home_06: list });
    } catch (e) { this.mpDevWarn('product', "outdoor_v05_home_06", e); }
  },
async loadSwipers() {
    await this._mpLoadSwiper_swiper_outdoor_v05_home_02()
  },
  async _mpLoadSwiper_swiper_outdoor_v05_home_02() {
    const { req, assetUrl } = require('../../utils/api');
    try {
      var j = await req('/swiper/list.php?id=' + encodeURIComponent("outdoor_v05_home_02"));
      ...
      var cur = this.data.swiper_outdoor_v05_home_02 || {};
      this.setData({
        swiper_outdoor_v05_home_02: {
    ...
    } catch (e) { this.mpDevWarn('swiper', "outdoor_v05_home_02", e); }
  },
noticeDuration(speed) {
  ...
  async loadNotices() {
    await this._mpLoadNotice_notice_outdoor_v05_home_03()
  },
  async _mpLoadNotice_notice_outdoor_v05_home_03() {
    const { req } = require('../../utils/api');
    try {
      var j = await req('/notice/get.php?id=' + encodeURIComponent("outdoor_v05_home_03"));
      if (!j || j.code !== 0 || !j.data || !j.data.content) return;
      var d = j.data;
      var speed = d.scrollSpeed || 50;
      var dir = d.scrollDirection || 'left';
      this.setData({
        notice_outdoor_v05_home_03: {
          content: d.content || '',
    ...
    } catch (e) { this.mpDevWarn('notice', "outdoor_v05_home_03", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_outdoor_v05_home_04, 'image'); if (list.length) this.setData({ products_outdoor_v05_home_04: list }); }
    { var list = withSrc(this.data.products_outdoor_v05_home_06, 'image'); if (list.length) this.setData({ products_outdoor_v05_home_06: list }); }
    { var wgt = this.data.swiper_outdoor_v05_home_02; if (wgt && wgt.items && wgt.items.length) { var next = Object.assign({}, wgt, { items: withSrc(wgt.items, 'image') }); this.setData({ swiper_outdoor_v05_home_02: next }); } }
  },
onShow() {
