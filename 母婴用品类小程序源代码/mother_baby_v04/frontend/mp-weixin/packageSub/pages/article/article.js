...
async loadSwipers() {
    await this._mpLoadSwiper_swiper_mother_baby_v04_article_03()
  },
  async _mpLoadSwiper_swiper_mother_baby_v04_article_03() {
    const { req, assetUrl } = require('../../../utils/api');
    try {
      var j = await req('/swiper/list.php?id=' + encodeURIComponent("mother_baby_v04_article_03"));
      ...
      var cur = this.data.swiper_mother_baby_v04_article_03 || {};
      this.setData({
        swiper_mother_baby_v04_article_03: {
    ...
    } catch (e) { this.mpDevWarn('swiper', "mother_baby_v04_article_03", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.articles_mother_baby_v04_article_02, 'cover'); if (list.length) this.setData({ articles_mother_baby_v04_article_02: list }); }
    { var wgt = this.data.swiper_mother_baby_v04_article_03; if (wgt && wgt.items && wgt.items.length) { var next = Object.assign({}, wgt, { items: withSrc(wgt.items, 'image') }); this.setData({ swiper_mother_baby_v04_article_03: next }); } }
  },
goArticle(e) {
...
async loadArticles() {
    ...
    try {
      await this._mpLoadArticles_articles_mother_baby_v04_article_02()
    } finally {
      this._articleLoading = false;
    }
  },
  async _mpLoadArticles_articles_mother_baby_v04_article_02() {
    const { req, assetUrl } = require('../../../utils/api');
    var url = '/article/list.php?limit=5&component_id=' + encodeURIComponent("mother_baby_v04_article_02");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.articles_mother_baby_v04_article_02 || []).slice();
      }
      if (!list.length) return;
      this.setData({ articles_mother_baby_v04_article_02: list });
    } catch (e) {
      this.mpDevWarn('article', "mother_baby_v04_article_02", e);
      var cur = this.data.articles_mother_baby_v04_article_02;
      if (cur && cur.length) {
        try {
          ...
          this.setData({ articles_mother_baby_v04_article_02: patched });
        } catch (e2) {}
      }
    }
  },
