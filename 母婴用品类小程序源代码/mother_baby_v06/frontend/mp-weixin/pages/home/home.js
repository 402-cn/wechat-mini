...
async loadProducts() {
    await this._mpLoadProducts_products_mother_baby_v06_home_05()
  },
  async _mpLoadProducts_products_mother_baby_v06_home_05() {
    const { req, assetUrl } = require('../../utils/api');
    var url = '/product/list.php?limit=4&component_id=' + encodeURIComponent("mother_baby_v06_home_05");
    try {
      var j = await req(url);
      var list = [];
      if (j && j.code === 0 && j.data && j.data.list && j.data.list.length) {
        ...
        list = (this.data.products_mother_baby_v06_home_05 || []).slice();
      }
      if (!list.length) return;
      this.setData({ products_mother_baby_v06_home_05: list });
    } catch (e) { this.mpDevWarn('product', "mother_baby_v06_home_05", e); }
  },
async loadSwipers() {
    await this._mpLoadSwiper_swiper_mother_baby_v06_home_02()
  },
  async _mpLoadSwiper_swiper_mother_baby_v06_home_02() {
    const { req, assetUrl } = require('../../utils/api');
    try {
      var j = await req('/swiper/list.php?id=' + encodeURIComponent("mother_baby_v06_home_02"));
      ...
      var cur = this.data.swiper_mother_baby_v06_home_02 || {};
      this.setData({
        swiper_mother_baby_v06_home_02: {
    ...
    } catch (e) { this.mpDevWarn('swiper', "mother_baby_v06_home_02", e); }
  },
...
seedDemoImages() {
    ...
    { var list = withSrc(this.data.products_mother_baby_v06_home_05, 'image'); if (list.length) this.setData({ products_mother_baby_v06_home_05: list }); }
    { var wgt = this.data.swiper_mother_baby_v06_home_02; if (wgt && wgt.items && wgt.items.length) { var next = Object.assign({}, wgt, { items: withSrc(wgt.items, 'image') }); this.setData({ swiper_mother_baby_v06_home_02: next }); } }
  },
onShow() {
